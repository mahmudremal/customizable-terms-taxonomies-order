...
class Ajax {
	...
	protected function setup_hooks() {
		add_action('wp_ajax_ctto/project/teddybearpopupaddon/action/get_autocomplete', [$this, 'get_autocomplete'], 10, 0);
		add_action('wp_ajax_nopriv_ctto/project/teddybearpopupaddon/action/get_autocomplete', [$this, 'get_autocomplete'], 10, 0);

		add_action('wp_ajax_ctto/project/ajax/search/product', [$this, 'search_product'], 10, 0);
		add_action('wp_ajax_nopriv_ctto/project/ajax/search/product', [$this, 'search_product'], 10, 0);
		add_action('wp_ajax_ctto/project/ajax/submit/popup', [$this, 'submit_popup'], 10, 0);
		add_action('wp_ajax_nopriv_ctto/project/ajax/submit/popup', [$this, 'submit_popup'], 10, 0);

		add_action('wp_ajax_ctto/project/ajax/edit/product', [$this, 'edit_product'], 10, 0);
		add_action('wp_ajax_ctto/project/ajax/save/product', [$this, 'save_product'], 10, 0);

		add_action('wp_ajax_ctto/project/ajax/search/popup', [$this, 'search_popup'], 10, 0);
		add_action('wp_ajax_nopriv_ctto/project/ajax/search/popup', [$this, 'search_popup'], 10, 0);
		
		add_action('wp_ajax_ctto/project/ajax/update/orderitem', [$this, 'update_orderitem'], 10, 0);

		add_action('wp_ajax_nopriv_ctto/project/ajax/suggested/names', [$this, 'suggested_names'], 10, 0);
		add_action('wp_ajax_ctto/project/ajax/suggested/names', [$this, 'suggested_names'], 10, 0);

		add_action('wp_ajax_nopriv_ctto/project/ajax/checkout/fetch', [$this, 'checkout_fetch'], 10, 0);
		add_action('wp_ajax_ctto/project/ajax/checkout/fetch', [$this, 'checkout_fetch'], 10, 0);
	...
	public function search_product() {
		global $wpdb;global  $woocommerce;global $ctto_Product;global $ctto_Plushies;
		// check_ajax_referer('ctto/project/teddybearpopupaddon/verify/nonce', '_nonce', true);
		...
		$json = [
			...
			'product' => [
				...
				'positions' => [
					'standing' => apply_filters('ctto/project/system/getoption', 'standard-standingdoll', ''),
					'sitting' => apply_filters('ctto/project/system/getoption', 'standard-sittingdoll', ''),
				],
				'is_parent' => false,
				...
				'custom_fields' => $ctto_Product->get_frontend_product_json($dataset['product_id'])
			],
		];

		if (!$json['product']['custom_fields'] || count($json['product']['custom_fields']) <= 0) {
			$json['product']['empty_image'] = CTTO_BUILD_URI . '/icons/Team meeting_Monochromatic.svg';
		}
	...
	public function submit_popup() {
		...
		$json = [
			...
			'message' => __('Popup submitted successfully. Hold on unil you\'re redirecting to searh results.', 'ctto')
		];
		...
		if (isset($request['field']["9002"]) && ! is_user_logged_in()) {
			...
			if ($user) {
				...
				if (!$user_id && false == email_exists($user_email)) {
					...
					$random_password = __('User already exists.  Password inherited.', 'ctto');
				}
	...
	public function save_product() {
		$result = [];
		$result['hooks'] = ['product_updated'];$result['message'] = __('Popup updated Successfully', 'ctto');
		$request = json_decode(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', stripslashes(html_entity_decode($_POST['dataset']))), true);
		$result['json'] = $request;
	...
	public function edit_product() {
		global $ctto_Product;$json = [];global $ctto_Plushies;// $ctto_Product->
		...
		foreach($ctto_Plushies->get_accessories_terms() as $_key => $_text) {
			$args['meta_query'][] = [
				'key' => $_key,
				'value' => 'on',
				'compare' => '='
	...
	public function merge_customfields($fields) {
		...
		if (!is_user_logged_in()) {
			...
			$fields[] = [
				...
				'footer'	=> sprintf(__('By continuing, you confirm your agreement to our %sTerms & Conditions%s', 'ctto'), '<a href="'.esc_url(get_privacy_policy_url()).'" target="_blank">', '</a>'),
				'headerbgurl' => false,
				'extra_fields' => [
					[
						'fieldID' => ($fieldID+1),
	...
	public function update_orderitem() {
		global $ctto_Product;global $ctto_Meta;
		$json = ['hooks' => ['order_item_update_failed'], 'message' => __('Something went wrong. Please review your request again.', 'ctto')];
		...
		foreach($order->get_items() as $item_id => $order_item) {
			...
			$custom_popup = $ctto_Product->get_order_pops_meta($order, $order_item, $product_id);
			foreach($custom_popup as $posI => $posRow) {
				foreach($posRow as $i => $field) {
					if ($field['type'] == 'info') {
						$item_meta_data = $ctto_Meta->get_order_item_dataset($order_item, $order);
							...
							foreach($iRow as $j => $jRow) {
								if ($field['steptitle'] == $jRow['title'] && $j == 0) {
									if (
										...
										$json['message'] = __('Successfully Updated your teddy bear name.', 'ctto');
	...
	public function suggested_names() {
		...
		$filteredKeys = array_keys(CTTO_OPTIONS);
		$filteredData = [];
		foreach($filteredKeys as $key) {
			if (strpos($key, 'teddy-name-') !== false) {
				$filteredData[] = CTTO_OPTIONS[$key];
			}
		}
		...
		if (apply_filters('ctto/project/system/isactive', 'names-randomize')) {
			shuffle($args['names']);
		}
	...
	public function checkout_fetch() {
		$json = ['hooks' => ['checkout-fetch-failed'], 'message' => __('Something went wrong. Please review your request again.', 'ctto')];
