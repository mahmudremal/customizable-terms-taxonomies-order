...
class Option {
	...
	protected function setup_hooks() {
		$this->file = CTTO_FILE__;
		$this->plugin_slug = 'ctto';
		$this->plugin_name = 'ctto';
		$this->textdomain = 'ctto'; // str_replace('_', '-', $plugin_slug);
		// Initialise settings
		...
		add_filter('ctto/project/system/getoption', [$this, 'getOption'], 10, 2);
		add_filter('ctto/project/system/isactive', [$this, 'isActive'], 10, 1);
	}
	/**
	 * Initialise settings
	 * @return void
	...
	public function init() {
		$this->general = (object) wp_parse_args(apply_filters('ctto/project/settings/general', []), [
			'page_title'					=> __('Popup Configuration.', 'ctto'),
			'menu_title'					=> __('Customized Popup', 'ctto'),
			'role'							=> 'manage_options',
			'slug'							=> $this->plugin_slug,
			'page_header'					=> __('Teddy bear customization popup.', 'ctto'),
			'page_subheader'				=> __('Customization popup configuration screen. Default values and truning on/off can be done from here.', 'ctto'),
			'no_password'					=> __('A password is required.', 'ctto'),
		]);
	}
	...
	public function add_settings_link($links) {
		$settings_link = '<a href="options-general.php?page='.$this->general->slug.'">' . __('Settings', 'ctto') . '</a>';
		// array_push($links, $settings_link);
		$links[] = $settings_link;
		$links[] = '<a href="https://www.fiverr.com/mahmud_remal/" target="_blank">' . __('Developer Support', 'ctto') . '</a>';
		return $links;
	}
	/**
	...
	private function settings_fields() {
		$settings = [];
		$settings = apply_filters('ctto/project/settings/fields', $settings);
		return $settings;
	}
	/**
	 * Options getter
	...
	public function display_field($args) {
		...
		switch($field['type']) {
			...
			case 'radio':
			case 'select_multi':
				$html .= apply_filters('ctto/project/settings/fields/label', '<br/><span class="description">' . $field['description'] . '</span>', $field);
			break;
			default:
				$html .= apply_filters('ctto/project/settings/fields/label', '<label for="' . esc_attr($field['id']) . '"><span class="description">' . $field['description'] . '</span></label>' . "\n", $field);
			break;
		}
		echo $html;
	}
	...
	public function getOption($option, $default) {
		return isset(CTTO_OPTIONS[$option])?CTTO_OPTIONS[$option]:$default;
	}
	/**
	...
	public function isActive($option) {
		return (isset(CTTO_OPTIONS[$option]) && CTTO_OPTIONS[$option] == 'on');
	}
}